...
FIELD_ALIASES: Dict[str, Sequence[str]] = {
    ...
    "cht_1_deg": ("cht_1_deg", "e1_cht1", "raw_e1_cht1"),
    "cht_2_deg": ("cht_2_deg", "e1_cht2", "raw_e1_cht2"),
    "cht_3_deg": ("cht_3_deg", "e1_cht3", "raw_e1_cht3"),
    "cht_4_deg": ("cht_4_deg", "e1_cht4", "raw_e1_cht4"),
    "egt_1_deg": ("egt_1_deg", "e1_egt1", "raw_e1_egt1"),
    "egt_2_deg": ("egt_2_deg", "e1_egt2", "raw_e1_egt2"),
    "egt_3_deg": ("egt_3_deg", "e1_egt3", "raw_e1_egt3"),
    "egt_4_deg": ("egt_4_deg", "e1_egt4", "raw_e1_egt4"),
    "cht_1_deg_c": ("cht_1_deg_c",),
    "cht_2_deg_c": ("cht_2_deg_c",),
    "cht_3_deg_c": ("cht_3_deg_c",),
    "cht_4_deg_c": ("cht_4_deg_c",),
    "egt_1_deg_c": ("egt_1_deg_c",),
    "egt_2_deg_c": ("egt_2_deg_c",),
    "egt_3_deg_c": ("egt_3_deg_c",),
    "egt_4_deg_c": ("egt_4_deg_c",),
    "cht_1_deg_f": ("cht_1_deg_f",),
    "cht_2_deg_f": ("cht_2_deg_f",),
    "cht_3_deg_f": ("cht_3_deg_f",),
    "cht_4_deg_f": ("cht_4_deg_f",),
    "egt_1_deg_f": ("egt_1_deg_f",),
    "egt_2_deg_f": ("egt_2_deg_f",),
    "egt_3_deg_f": ("egt_3_deg_f",),
    "egt_4_deg_f": ("egt_4_deg_f",),
...
TELEMETRY_LOG_FIELDS = [
    ...
    "garmin_record_type",
    *TELEMETRY_FIELDS,
    "garmin_ias_raw_field",
    "garmin_rpm_raw_field",
    "garmin_checksum",
    "gps_position_status",
    "gps_horizontal_error_m",
    "garmin_eis_values",
    *[f"eis_sensor_{sensor_id.lower()}" for sensor_id in EIS_SENSOR_IDS_TO_LOG],
    "garmin_gps_altitude_m_raw_field",
    "garmin_gps_ew_velocity_raw_field",
    "garmin_gps_ns_velocity_raw_field",
    "garmin_gps_vertical_velocity_raw_field",
    "raw_serial_line",
    "payload_bytes",
    "payload_json",
]
...
DASHBOARD_FIELDS = [
    ("sequence", "seq", 5, 0),
    ("garmin_record_type", "rec", 4, None),
    ("ias_kt", "IAS", 7, 1),
    ("pitch_deg", "pitch", 7, 1),
    ("roll_deg", "roll", 7, 1),
    ("heading_deg", "hdg", 5, 0),
    ("rpm", "RPM", 6, 0),
    ("cht_1_deg_f", "CHT1F", 6, 0),
    ("cht_2_deg_f", "CHT2F", 6, 0),
    ("cht_3_deg_f", "CHT3F", 6, 0),
    ("cht_4_deg_f", "CHT4F", 6, 0),
    ("egt_1_deg_f", "EGT1F", 6, 0),
    ("egt_2_deg_f", "EGT2F", 6, 0),
    ("egt_3_deg_f", "EGT3F", 6, 0),
    ("egt_4_deg_f", "EGT4F", 6, 0),
    ("vertical_speed_fpm", "VS", 7, 0),
    ("gps_ground_speed_kt", "GS", 7, 1),
    ("gps_altitude_ft", "GPSalt", 8, 0),
]
...
def parse_garmin_text_line(line: str) -> Dict[str, Any] | None:
    ...
    if stripped.startswith("=3") and len(stripped) >= 57:
        temp_1_deg_c = parse_int_field(stripped[83:87]) if len(stripped) >= 87 else None
        temp_2_deg_c = parse_int_field(stripped[87:91]) if len(stripped) >= 91 else None
        temp_3_deg_c = parse_int_field(stripped[91:95]) if len(stripped) >= 95 else None
        temp_4_deg_c = parse_int_field(stripped[95:99]) if len(stripped) >= 99 else None
        temp_5_deg_c = parse_int_field(stripped[99:103]) if len(stripped) >= 103 else None
        temp_6_deg_c = parse_int_field(stripped[103:107]) if len(stripped) >= 107 else None
        temp_7_deg_c = parse_int_field(stripped[107:111]) if len(stripped) >= 111 else None
        temp_8_deg_c = parse_int_field(stripped[111:115]) if len(stripped) >= 115 else None
        # For this aircraft, the Text Out temp block matches the cockpit display
        # best as interleaved CHT/EGT channels in reverse cylinder-display order.
        cht_1_deg_c = temp_7_deg_c
        cht_2_deg_c = temp_5_deg_c
        cht_3_deg_c = temp_3_deg_c
        cht_4_deg_c = temp_1_deg_c
        egt_1_deg_c = temp_8_deg_c
        egt_2_deg_c = temp_6_deg_c
        egt_3_deg_c = temp_4_deg_c
        egt_4_deg_c = temp_2_deg_c
        row = {
            ...
            # Garmin Text Out engine temps are emitted in C; the cockpit commonly displays F.
            "cht_1_deg": cht_1_deg_c,
            "cht_2_deg": cht_2_deg_c,
            "cht_3_deg": cht_3_deg_c,
            "cht_4_deg": cht_4_deg_c,
            "egt_1_deg": egt_1_deg_c,
            "egt_2_deg": egt_2_deg_c,
            "egt_3_deg": egt_3_deg_c,
            "egt_4_deg": egt_4_deg_c,
            "cht_1_deg_c": cht_1_deg_c,
            "cht_2_deg_c": cht_2_deg_c,
            "cht_3_deg_c": cht_3_deg_c,
            "cht_4_deg_c": cht_4_deg_c,
            ...
            "egt_3_deg_c": egt_3_deg_c,
            "egt_4_deg_c": egt_4_deg_c,
            "cht_1_deg_f": celsius_to_fahrenheit(cht_1_deg_c),
            "cht_2_deg_f": celsius_to_fahrenheit(cht_2_deg_c),
            "cht_3_deg_f": celsius_to_fahrenheit(cht_3_deg_c),
            "cht_4_deg_f": celsius_to_fahrenheit(cht_4_deg_c),
            "egt_1_deg_f": celsius_to_fahrenheit(egt_1_deg_c),
            "egt_2_deg_f": celsius_to_fahrenheit(egt_2_deg_c),
            "egt_3_deg_f": celsius_to_fahrenheit(egt_3_deg_c),
            "egt_4_deg_f": celsius_to_fahrenheit(egt_4_deg_c),
        }
        field_slices = {
        ...
        if len(stripped) >= 115:
            field_slices.update(
                {
                    "cht_1_deg": stripped[107:111],
                    "cht_2_deg": stripped[99:103],
                    "cht_3_deg": stripped[91:95],
                    "cht_4_deg": stripped[83:87],
                    "egt_1_deg": stripped[111:115],
                    "egt_2_deg": stripped[103:107],
                    "egt_3_deg": stripped[95:99],
                    "egt_4_deg": stripped[87:91],
                    "cht_1_deg_c": stripped[107:111],
                    "cht_2_deg_c": stripped[99:103],
                    "cht_3_deg_c": stripped[91:95],
                    "cht_4_deg_c": stripped[83:87],
                    "egt_1_deg_c": stripped[111:115],
                    "egt_2_deg_c": stripped[103:107],
                    "egt_3_deg_c": stripped[95:99],
                    "egt_4_deg_c": stripped[87:91],
                    "cht_1_deg_f": stripped[107:111],
                    "cht_2_deg_f": stripped[99:103],
                    "cht_3_deg_f": stripped[91:95],
                    "cht_4_deg_f": stripped[83:87],
                    "egt_1_deg_f": stripped[111:115],
                    "egt_2_deg_f": stripped[103:107],
                    "egt_3_deg_f": stripped[95:99],
                    "egt_4_deg_f": stripped[87:91],
                }
            )
        row["_clear_stale_fields"] = [field for field, text in field_slices.items() if invalid_numeric_field(text)]
        return row
    ...
    if stripped.startswith("=5"):
        ...
        if eis_values:
            ...
            for sensor_id, value in eis_values.items():
                row[f"eis_sensor_{sensor_id.lower()}"] = value
        return row
...
def format_dashboard_cell(value: Any, width: int, digits: int | None) -> str:
    if value is None or value == "":
        ...
        try:
            number = float(value)
            text = "--" if not math.isfinite(number) else f"{number:.{digits}f}"
        except (TypeError, ValueError):
            text = str(value)
    if len(text) > width:
        text = text[:width]
...
def dashboard_header() -> str:
    return " ".join(label.rjust(width) for _, label, width, _ in DASHBOARD_FIELDS)


def print_dashboard_row(sequence: int, payload: Mapping[str, Any], repeat_header: bool) -> None:
    if repeat_header:
    ...
    print(
        " ".join(
            format_dashboard_cell(values.get(field), width, digits)
            for field, _, width, digits in DASHBOARD_FIELDS
        )
    )
...
def main() -> None:
    ...
    parser.add_argument("--baud", type=int, default=115200, help="Serial baud rate. Garmin G3X Text Out is 115200.")
    ...
    parser.add_argument(
        "--telemetry-log",
        default="outputs/garmin_rs232_udp_telemetry.csv",
        help="Wide CSV log of decoded telemetry fields for Excel/pandas monitoring.",
    )
    ...
    parser.add_argument("--dashboard", action="store_true", help="Print a live decoded telemetry table on the laptop.")
    parser.add_argument("--dashboard-every", type=int, default=1, help="Print one dashboard row every N forwarded packets.")
    ...
    telemetry_log_path = repo_path(args.telemetry_log)
    ...
    print(f"Telemetry CSV: {telemetry_log_path}")
    ...
    sent = 0
    sequence = 0
    dashboard_every = max(1, int(args.dashboard_every))
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
        ...
        with open(log_path, "w", encoding="utf-8", newline="") as log_handle, open(
            telemetry_log_path, "w", encoding="utf-8", newline=""
        ) as telemetry_log_handle:
            writer = csv.DictWriter(log_handle, fieldnames=log_fields, lineterminator="\n")
            telemetry_writer = csv.DictWriter(
                telemetry_log_handle,
                fieldnames=TELEMETRY_LOG_FIELDS,
                lineterminator="\n",
                extrasaction="ignore",
            )
            writer.writeheader()
            ...
            try:
                for line in line_source:
                    ...
                    log_handle.flush()
                    telemetry_log_handle.flush()
                    if args.print_packets:
                        print_live_packet(sequence, payload, line, args.print_raw)
                    if args.dashboard and sequence % dashboard_every == 0:
                        print_dashboard_row(sequence, payload, repeat_header=(sequence % (dashboard_every * 20) == 0))
                    sent += 1
                    sequence += 1
